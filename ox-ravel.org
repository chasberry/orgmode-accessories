...
#+begin_src emacs-lisp
  ...
  (defun org-ravel--yaml-header (info)
    ...
    (let ((rmd_yaml (plist-get info :with-auto-yaml-header)))
      (when rmd_yaml 
	(let* ((with-title  (plist-get info :with-title))
	       (with-author  (plist-get info :with-author))
	...
	(concat
	 (when (or (< 0 (length auto-content)) (<  0 (length yaml-export-blocks)))
	   (concat "---\n" "# YAML header created by ox-ravel\n"
		   (when auto-first (cdr rmd_yaml))
		   yaml-export-blocks
		   (when auto-last (cdr rmd_yaml))
		   "---\n"))
	 contents)))
...
The `rmd' backend and its child, `Ravel-markdown', will extract all
`yaml' export blocks and combine their contents to form a yaml header.

The `:with-auto-yaml-header' option controls automatic setting of
author, title and date. Setting `rmd_yaml:header' in an options line
will insert a yaml header with title, author, and date before any
other yaml lines.  Setting `rmd_yaml:footer' puts them after those
lines. Since yaml obeys `left-join' rules, the latter allows yaml
export blocks to override those settings.  Setting `rmd_yaml:nil'
prevents those lines from being inserted, which if there are no yaml
export blocks will result in no yaml header being included.
...
#+BEGIN_SRC emacs-lisp :comments org
  ...
  (eval-after-load 'ox-md
    '(progn
	 ...
	 :filters-alist   '((:filter-latex-fragment .org-ravel-cite-as-pandoc)))
       (org-ravel-define-exporter
	'ravel-markdown
