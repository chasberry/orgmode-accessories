...
#+BEGIN_SRC emacs-lisp
  ...
  `org-ravel-export-to-file' and similar actions need to redefine
   ...
   to be ravel compliant, and (by default) turn off confirmation
   for the evaluation of ravel blocks.

  `(macroexpand (org-ravel-export-wrapper BODY))' when used inside a `defun' will take care of these issues."
...
#+BEGIN_SRC emacs-lisp
  ...
  CONTENTS is nil.  INFO is a plist holding contextual information."
	...
	(org-element-property :value export-block)
      (let ((parent-backend (org-export-backend-parent
			     (org-element-property :back-end export-block)))) 
	(if parent-backend
