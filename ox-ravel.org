...
#+begin_src emacs-lisp
  ...
  (defun org-ravel-rmd-template (contents info)
    ...
    (let* ((rmd_yaml (org-ravel--yaml-header info))
	   ...
	   (bibliog (org-element-map parsed 'link
		      (lambda (lnk) (and
				     (string= "bibliography"
					      (org-element-property
					       :type lnk))
				     (org-element-property :path lnk)))))
	   (biblines (when bibliog (mapconcat
				    '(lambda (x) (replace-regexp-in-string
						 "[.]bib$" "" x))
				    (cons "bibliography: " bibliog)
				    "\n  - ")))
	   (yaml-export-blocks
  ...
  a communication channel."
    (unless (string= (org-element-property :type link) "bibliography")
