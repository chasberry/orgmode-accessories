...
*** TODO R markdown exports
This is setup to use org-ref.  I think I need to use (featurep
'org-ref) or maybe (eval-after-load 'org-ref) to set this as an
option.

Maybe need to set it up as optional, so users can opt out.
...
`:with-auto-yaml-header' option controls inclusion of the author,
title and date (from the corresponding =org= options) in that
...
LaTeX style =\cite= directives are translated to Pandoc format in ='ravel-markdown=.

The option =:with-biblinks= allows bibliography links to be processed
if such are defined.  Often one will want to place the bibliography
directives in a =YAML= export block, and the option =biblinks:nil= will
prevent superfluous insertion of a bibliography in the =*.Rmd= file.
  ...
  (defun org-rmd-link (link contents info)
    ...
    (unless (and 
	     (string= (org-element-property :type link) "bibliography")
	      (not (plist-get info :with-biblinks)))
      (or (org-export-custom-protocol-maybe link contents 'rmd)
