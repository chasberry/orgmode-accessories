...
Arbitrary YAML arguments in ~#+ATTR_RAVEL~ properties can be enclosed
in backticks. These will be stripped on output. Interior backticks
will not be affected.

Note that ~:ravel~ header arguments are a bit fragile. Their names
cannot include special characters like backticks or dashes unless they
occur in lisp expressions, e.g. ~(identity `code-fold`=TRUE)~. So,
ordinarily the ~#+ATTR_RAVEL~ idiom is used to pass cell options like ~case-fold~.

Malformed names may be ignored on output.  There is a check that non-nil
properties and ~:ravel~ headers produce a result, but this falls short
of checking that each and every specification is proper.
