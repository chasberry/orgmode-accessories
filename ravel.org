...
- so here is what I am figuring out:
  ...
  - NB ( assoc 'src-block (reverse org-e-rnw-translate-alist)) will
...
With ~e-ravel-el~ loaded and the point in a =*.org= buffer, 
...
- ~e-latex-brew~ :: LaTeX brew templates
...
#+begin_src emacs-lisp :tangle e-ravel.el
  (org-export-define-derived-backend e-latex-noweb e-latex
    ...
    :menu-entry
    (?l 1
        ((?r "As Rnw File" org-ravel-e-latex-noweb-dispatch))))
#+end_src
...
#+BEGIN_SRC emacs-lisp :tangle e-ravel.el :results silent
  (defun org-ravel-e-latex-noweb-dispatch 
  ...
  (if async
    ...
    (let
        ((outfile  (org-export-output-file-name ".Rnw" subtreep)))
         (org-export-to-file 'e-latex-noweb 
...
#+BEGIN_SRC emacs-lisp :tangle e-ravel.el :results silent
  (unless 
      (assoc "minimalRnw" org-e-latex-classes)
    (let    
        ((art-class (assoc "article" org-e-latex-classes))
...
:DEVNOTES:
   to reset this backend after first execution
   use (makunbound 'org-e-beamer-noweb-translate-alist)
:END:
...
#+begin_src emacs-lisp :tangle e-ravel.el
  (org-export-define-derived-backend e-beamer-noweb e-beamer
    ...
    :menu-entry
    (?l 1
        ((?s "As Rnw Beamer" org-ravel-e-beamer-noweb-dispatch))))
#+end_src
...
See the comments for ~e-latex-noweb~.
...
#+BEGIN_SRC emacs-lisp :tangle e-ravel.el :results silent
  (defun org-ravel-e-beamer-noweb-dispatch 
    (&optional async subtreep visible-only body-only ext-plist)
  "Execute menu selection. See org-export.el for meaning of ASYNC,
        SUBTREEP, VISIBLE-ONLY and BODY-ONLY."
  (interactive)
  (if async
    ...
    (let
        ((outfile  (org-export-output-file-name ".Rnw" subtreep)))
         (org-export-to-file 'e-beamer-noweb 
                             outfile subtreep visible-only 
                             body-only ext-plist))))
#+END_SRC
...
:DEVNOTES:
   to reset this backend after first execution
   use (makunbound 'org-e-latex-brew-translate-alist)
:END:
...
#+BEGIN_SRC emacs-lisp :tangle e-ravel.el
  (org-export-define-derived-backend e-latex-brew e-latex
     ...
     :menu-entry
     (?l 2
        ((?w "As Brew File" org-ravel-e-latex-brew-dispatch))))
...
See the comments for ~e-latex-noweb~. The =minimalRnw= class can be
used here, too.
...
#+BEGIN_SRC emacs-lisp :tangle e-ravel.el :results silent
  (defun org-ravel-e-latex-brew-dispatch 
    (&optional async subtreep visible-only body-only ext-plist)
  "Execute menu selection. See org-export.el for meaning of ASYNC,
        SUBTREEP, VISIBLE-ONLY and BODY-ONLY."
  (interactive)
  (if async
    ...
    (let
        ((outfile  (org-export-output-file-name ".brew" subtreep)))
         (org-export-to-file 'e-latex-brew 
                             outfile subtreep visible-only 
                             body-only ext-plist))))
#+END_SRC
...
#+begin_src emacs-lisp :tangle e-ravel.el
  (org-export-define-derived-backend e-html-knitr e-html
    :translate-alist ((src-block . org-ravel-src-block)
     ...
     :menu-entry
     (?h 3
        ((?r "As Rhtml File" org-ravel-e-html-knitr-dispatch))))
...
#+BEGIN_SRC emacs-lisp  :tangle e-ravel.el :results silent
  (defun org-ravel-e-html-knitr-dispatch 
    (&optional async subtreep visible-only body-only ext-plist)
  "Execute menu selection. See org-export.el for meaning of ASYNC,
        SUBTREEP, VISIBLE-ONLY and BODY-ONLY."
  (interactive)
  (if async
    ...
    (let
        ((outfile  (org-export-output-file-name ".Rhtml" subtreep)))
         (org-export-to-file 'e-html-knitr 
                             outfile subtreep visible-only 
                             body-only ext-plist))))
#+END_SRC
** md-knitr backend
...
The function ~org-ravel-chunk-style-e=latex-brew~ (defined above)
works.

#+name: md-brew-chunk-style
#+BEGIN_SRC emacs-lisp :tangle e-ravel.el
  (defalias 'org-ravel-chunk-style-md-brew  
    'org-ravel-chunk-style-e-latex-brew)
#+END_SRC

*** inline src style

The function ~org-ravel-inline-style-e-latex-brew~ (defined above)
works.

#+name: md-brew-chunk-style
#+BEGIN_SRC emacs-lisp :tangle e-ravel.el
  (defalias 'org-ravel-inline-style-md-brew  
    'org-ravel-inline-style-e-latex-brew)
#+END_SRC

*** export dispatcher
    
    See the comments for ~e-latex-brew~.
...
#+BEGIN_SRC emacs-lisp  :tangle e-ravel.el
  (defun org-ravel-export-e-latex-noweb 
    (outfile &optional subtree only-contents visible-only)
  ...
  VISIBLE-ONLY as per `org-export-as'."  
    (cond
     ...
     ((member only-contents 
      ...
      (flet 
        ...
        (let* ((nsection (if (member only-contents '(t "t"))
                             1 
                           (if (stringp only-contents) 
                               (string-to-number only-contents)
                             only-contents)))
          ...
          (org-export-to-file 'e-latex-noweb outfile subtree visible-only))))
...
** COMMENT export to docx

export to markdown - Rmd
then knit the result
then follow [[http://stackoverflow.com/questions/19889459/knitr-html-to-word-docx-using-pandoc][knitr to docx post]]
