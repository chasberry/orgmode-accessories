...
* COMMENT knitr to org


The code chunks can be extracted by ~purl(...)~. In fact, one could
copy them and replace them with comments.
...
#+BEGIN_SRC R :results output
    ...
    doctext <-
        paste(
            "a\n<<b,echo=FALSE,opt1=1,opt2=2,opt3=3,opt4=4>>=",
            "ls()\n@\nmore text goes here",
            "an inline expr \\Sexpr{1+2} was here",
            "<<>>=\nunlabelled <- TRUE\n@",
            sep="\n")
  cat(doctext,"\n")
  cat(
...
Limited experimentation shows that at least four dashes will be at the
end of the line with the label and options.

Lines with no labels or options are all dashes.

In line expressions do not get parsed, it appears. So there is still
some work to do to translate them back to src_R{} blocks.

Easy-peasy.

But turning md back to org might have a few hiccups

* COMMENT tables to R

[[http://article.gmane.org/gmane.emacs.orgmode/51153][org mode tables and tangling thread]]

[[http://permalink.gmane.org/gmane.emacs.orgmode/81248][another such thread]]
