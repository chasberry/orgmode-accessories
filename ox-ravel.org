...
The `rmd' backend and its child, `Ravel-markdown', have the
`:with-auto-yaml-header' option. Setting `rmd_yaml:header' in an
options line will insert a yaml header with title, author, and
date. Doing so may interfere with later yaml blocks in report
generators like the R markdown::render() function that evidently
expects particular setting to apper in the very first YAML block. So
the default is set to `footer'.
...
#+BEGIN_SRC emacs-lisp :comments org
  ...
  (defun org-ravel-rmd-template (contents info)
    ...
    as a communication channel. A YAML block is added as a header or footer."
    (let ((rmd_yaml (org-ravel--yaml-header info)))
      (concat (when (eq (car rmd_yaml) 'header) (cdr rmd_yaml))
	      contents
	      (when (eq (car rmd_yaml) 'footer) (cdr rmd_yaml)))))
