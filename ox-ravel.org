...
`:with-auto-yaml-header' option. Setting `rmd_yaml:t' in an options
line will insert a yaml header with title, author, and date. Doing so
may interfere with later yaml blocks in some report generators. 
...
#+BEGIN_SRC emacs-lisp :comments org
  ...
  (defun org-ravel--yaml-header (info)
    "A minimal header."
    (when (plist-get info :with-auto-yaml-header)
  ...
  as a communication channel."
    (concat (org-ravel--yaml-header info) contents))
  ...
  (eval-after-load 'ox-md
	...
	:options-alist '((:with-auto-yaml-header nil "rmd_yaml" nil t)) 
