...
#+BEGIN_SRC emacs-lisp
  ...
  Set up a simple `defun' with NAME as a string and some ARGLIST, and
    format BODY as the docstring.  See `org-ravel-style-x'."
...
#+BEGIN_SRC emacs-lisp
  ...
  `org-ravel-style' or the default for BACKEND will be used.  This
  can be run by Babel using a backend that is not ravel-compliant
...
#+BEGIN_SRC emacs-lisp
  ...
  "Construct an action for RAVEL-BKEND.

  Users should not invoke this function directly.

  `Action' is in the sense of `org-export-define-backend' (see its
  docstring).  `org-ravel-define-exporter' calls this function and
  its docstring describes the other arguments STYLE-DEF (AULT),
  FILE-FUN, BUF-FUN and POST-PROC.  RAVEL-BKEND must be a quoted
  symbol when evaluated, which is automatic when used by
  `org-ravel-define-exporter'."
