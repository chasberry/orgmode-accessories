...
** Load the new exporter files
   ...
   defined backends. Here I use =e-latex=:
   #+name: loadup
   #+begin_src emacs-lisp :results output silent
  (require 'org-element)
  (require 'org-export)
...
- exporting brew style to *.org might be an awesome capability. Here is how to do it:
  - write the usual code including `:ravel <%[=] ... [{}][-]%>' arguments
...
- so here is what I am figuring out:
  - need to advise org-babel-exp-do-export
  ...
  - define a universal src-block function
  - define a universal inline-src-block function
  - define a back-end specific src-block format
  - define a back-end specific inline-src-block format
  - for the non-R src block function, use the ancestral version
  - for the non-R inline src block function, use ancestor
  - ( assoc 'src-block (reverse org-e-rnw-translate-alist)) will find
    the ancestor!!!!
  - org-export-before-parsing-hook is handy - it can remove delimiters
...
Currently, backends for ~e-latex-noweb~ and ~e-latex-brew~ are
...
other backends.It might help to prefix arguments with the name of
their backend.
...
be (re-)rendered and add the necessary arguemtns to the output string
